...
main = do
  ...
  putStrLn $ "  dimap (map (*2)) sum [1,2,3]: " ++ show (dimap (map (*2)) sum id ([1,2,3] :: [Int]))
  let person = PersonP { _nameP = "Alice", _ageP = 30 }
  putStrLn $ "  原始: " ++ show person
  putStrLn $ "  view name: " ++ viewP nameLensP person
  putStrLn $ "  view age:  " ++ show (viewP ageLensP person)
  putStrLn $ "  over name (++\" Smith\"): " ++ show (overP nameLensP (++" Smith") person)
  putStrLn $ "  set age 31: " ++ show (setP ageLensP 31 person)
  putStrLn ""

  -- ---- Arrow vs Monad 对比 ----
  putStrLn "=== 5. Arrow vs Monad 对比 ==="
  putStrLn "  Monad:"
  putStrLn "    * 顺序执行: do { x <- f; g x }"
  putStrLn "    * 输出可以影响下一步的输入"
  putStrLn "    * 更强大，但更受限（需要特定结构）"
  putStrLn ""
  putStrLn "  Arrow:"
  putStrLn "    * 静态连接图: f >>> g (编译时确定)"
  putStrLn "    * 支持并行 (**) 和分叉 (&&&)"
  putStrLn "    * 更通用，可表示电路、数据流、自动机"
  putStrLn "    * 每个 Monad 都可以通过 Kleisli 成为 Arrow"
  putStrLn ""
  putStrLn "  选择建议:"
  putStrLn "    * 需要动态分支/依赖? -> Monad"
  putStrLn "    * 需要静态分析/并行化? -> Arrow"
  putStrLn ""
